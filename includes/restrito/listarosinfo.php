...
if(!empty($retorno->reposicoes)){
  ...
  $saida .= '
    ...
    <select name="reposicoes[]" id="reposicoes" data-placeholder="Nada foi encontrado" multiple>
  ';
  foreach($resultadosReposicoes as $resultadoReposicao) {
    if(in_array($resultadoReposicao['id_peca'], $reposicoesExplode)){
      $saida .= '
        <option value="'.$resultadoReposicao['id_peca'].'" selected>'.$resultadoReposicao['descricao'].'</option>
      ';
    }else{
      $saida .= '
        <option value="'.$resultadoReposicao['id_peca'].'">'.$resultadoReposicao['descricao'].'</option>
      ';
    }
  }
  $saida .= '
...
$saida .= '
  ...
  <span><b>Observações Suporte</b></span>
  <textarea type="text" class="form-control" name="observacoes" id="textareaObservacoes"></textarea>
  </div>
';
if($retorno->id_status === '1'){
  $saida .= '
    <div class="col-md-12 mb-3">
    <input type="hidden" name="id_protocolo" value="'.$retorno->id.'">
    <input type="hidden" name="datahora" value="'.$retorno->datahora.'">
    <input type="hidden" name="email" value="'.$retorno->email.'">
    <input type="hidden" name="acao" value="iniciarAtendimento">
    <button type="submit" class="btn btn-info">Iniciar Atendimento</button>
    </div>
    </form>
    </div>
  ';
}elseif($retorno->id_status === '2' || $retorno->id_status === '10' || $retorno->id_status === '11'){
  $saida .= '
    <div class="col-md-12 mb-3">
    <input type="hidden" name="id_protocolo" value="'.$retorno->id.'">
    <input type="hidden" name="email" value="'.$retorno->email.'">
    <input type="hidden" name="observacoesantigasti" value="'.$retorno->observacoes.'">
    <input type="hidden" name="acao" value="atualizarHistorico">
    <button type="submit" class="btn btn-info">Atualizar Histórico</button>
    </div>
    </form>
    </div>
  ';
...
if($retorno->id_status === '1'){
  ...
  $saida .= '
    ...
    <input type="hidden" name="id_protocolo" value="'.$retorno->id.'">
    <input type="hidden" name="email" value="'.$retorno->email.'">
    <input type="hidden" name="acao" value="finalizarAtendimento">
